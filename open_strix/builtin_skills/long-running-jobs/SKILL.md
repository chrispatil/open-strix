...
7. **`shell_jobs_list` status lags the wake-up events that actually deliver results to you.** A job's `exited_ok`/`status` field in `shell_jobs_list` can flip to "done" before the corresponding `shell_job_complete` notification has actually reached you as a wake-up turn — the two are decoupled. If you're trying to confirm "no more stragglers" before wrapping up a batch of async jobs, don't trust a `shell_jobs_list` snapshot for that; instead track the count of *distinct job_ids you've actually been woken up for* via `shell_job_complete` events this session against the count of jobs you actually launched. Declaring completion off `shell_jobs_list` status alone risks a false "all done" a turn or two before the last straggler's wake-up actually lands.
